  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        index_property++;
        //divido già per il numero di particelle perchè alla fine voglio U/N
...
void System :: measure(){ // Measure properties
  ...
  if (_measure_penergy or _measure_pressure or _measure_gofr) {
    for (int i=0; i<_npart-1; i++){
      for (int j=i+1; j<_npart; j++){
        ...
        if (_measure_gofr) {
          int i_bin = dr/double(_bin_size);
          if (i_bin<_n_bins) _measurement(_index_gofr+i_bin) += 2.;
        }
  ...
  if (_measure_gofr){
    for (int i_bin=0; i_bin<_n_bins; i_bin++) {
      double r_i = _bin_size * i_bin;
      double DeltaV = ((4.*M_PI)/3.)*(pow(r_i+_bin_size,3)-pow(r_i,3));
      _measurement(_index_gofr+i_bin) /= _rho*_npart*DeltaV;
   }
  }
  ...
  if (_measure_pressure) {
    if (_sim_type==0) _measurement[_index_pressure] = _rho * (2.0/3.0) * kenergy_temp + (48.0*virial/3.0)/_volume + _ptail;
    if (_sim_type==1) _measurement[_index_pressure] = _rho * _temp + (48.0*virial/3.0)/_volume + _ptail; 
  }
  // MAGNETIZATION /////////////////////////////////////////////////////////////
// TO BE FIXED IN EXERCISE 6
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
...
void System :: averages(int blk){
  ...
  if (_measure_gofr){
    ...
    for (int bin = 0; bin < _n_bins; bin++){
      average     = _average(_index_gofr + bin);       // media del blocco per il bin,
      sum_average = _global_av(_index_gofr + bin);     // somma cumulativa 
      sum_ave2    = _global_av2(_index_gofr + bin) ; 
  
      double r_bin = (bin + 0.5) * _bin_size;  // centro del bin
  
      coutf << setw(12) << blk  //indice del blocco 
      << setw(12) << r_bin        // ascissa 
