...
private:
  ...
  bool _restart;        // Flag indicating if the simulation is restarted. In c++ 0=false. RESTART=TRUE SIGNIFICA CHE RIPARTO DALLA SIMULAZIONE PRECEDENTE, RESTART=FALSE PARTO DA ZERO
...
public: // Function declarations
  ...
  void ReverseVerlet();       // Inverte configurazione attuale e passata e inverte la velocità per tornare indietro
  double Force(int i, int dim); // Calculate force on a particle along a dimension //calcola la componente lungo dim della forza (Lennard-Jones) agente sulla particella i-esima, usando un cutoff sferico _r_cut
